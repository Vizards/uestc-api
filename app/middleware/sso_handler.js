...
module.exports = () => {
  return async (ctx, next) => {
    ...
    const check = () => ctx.helper.checkSSO('http://eams.uestc.edu.cn/eams/home.action', cookies, 'eams.uestc.edu.cn');
    try {
      let res = await check();
      ...
      if (res.body.includes('您的当前位置')) {
        ctx.locals.user.data.cookies = cookies.substr(0, 140);
        return next();
      }
      ...
      if (res.headers['set-cookie'][0].includes('route')) {
        cookies = `${res.headers['set-cookie'][0]};${cookies}`;
      }
      ...
      while (res.body === '' && res.headers.location.includes('ticket')) {
        ...
        if (res.headers['set-cookie'].length === 2) {
          cookies = `${cookies.substr(0, 39)}${res.headers['set-cookie'][0]};${res.headers['set-cookie'][1]};`;
        }
      }
      // 在此处触发重复登录的界面
      res = await check();
